...
class CanteraFS(VariableTree):
    ...
    def setStatic(self):
    	    ...
    	    if self._mach_or_area  == 0:
    	    	    ...
    	    	    self.Ps = self.Pt / ( 1 + ( self.gamt - 1. ) / 2. *self.Mach**2. )**( self.gamt  / ( self.gamt - 1. ))
                    self.MachTemp = 0
                    def eval(Ps):
                        self.Ps = Ps
                    	self._flowS = self._flow 
                    ...
                    newton(eval, self.Ps)
                    print "Mach = %s, MachTemp = %s"%(self.Mach,self.MachTemp)
                    areaT = self.W / ( self.rhos * self.Vflow ) * 144.
	    	    ...
	    	    def F1(Mach):
	    	    	 ...
	    	    	 self._mach_or_area = 1
	    	    	 self.setStatic()
	    	    	 print "root" ,  self.W / ( self.rhos * self.Vflow ) * 144., self.area
	    	    	 return self.W / ( self.rhos * self.Vflow ) * 144. - self.area
	    	    def DF1( Mach ):
	    	    	 self.Mach = Mach
	    	    	 self._mach_or_area = 1
	    	    	 self.setStatic()
	    	    	 area1 = self.W / ( self.rhos * self.Vflow ) * 144.
	    	    	 self.Mach = Mach +.01
	    	    	 self.setStatic()
	    	    	 print "return value ", self.W / ( self.rhos * self.Vflow ) * 144. - area1
	    	    	 return self.W / ( self.rhos * self.Vflow ) * 144. - area1
	    	    newton( F1, Mach )
	    	    self._mach_or_area = 2
